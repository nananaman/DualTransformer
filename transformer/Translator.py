...
class Translator(object):
    ...
    def __init__(self, opt, model=None, device=None):
        ...
        if model is None:
            ...
            self.model_opt = self.opt

        self.model = model
        self.model.word_prob_prj = nn.LogSoftmax(dim=1)
        self.model.eval()

    def translate_batch(self, src_seq, src_pos, domain):
        ...
        def beam_decode_step(
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm):
                if str(type(self.model)) == "<class 'torch.nn.parallel.data_parallel.DataParallel'>":
                    dec_output, * \
                        _ = self.model.module.decoder(
                            dec_seq, dec_pos, src_seq, enc_output)
                    # Pick the last step: (bh * bm) * d_h
                    dec_output = dec_output[:, -1, :]
                    word_prob = F.log_softmax(
                        self.model.module.tgt_word_prj(dec_output), dim=1)
                else:
                    dec_output, * \
        ...
        with torch.no_grad():
            ...
            if str(type(self.model)) == "<class 'torch.nn.parallel.data_parallel.DataParallel'>":
                src_enc, *_ = self.model.module.encoder(src_seq, src_pos)
            else:
            ...
            #-- Prepare beams
            inst_dec_beams = [Beam(n_bm, domain, device=self.device)
